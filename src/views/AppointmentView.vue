        ...
        <div class="max-w-3xl mx-auto glass-panel p-8 md:p-10 rounded-2xl shadow-glass-lg">
          ...
          <form @submit.prevent="submitAppointment" class="space-y-6">
            ...
            <p class="text-xs text-gray-400 text-center mt-3">
              Submitting will open your email client with the appointment details pre-filled. Well confirm the slot by email.
            </p>
          </form>
        </div>
      </div>
...
const submitAppointment = () => {
  ...
  const subject = encodeURIComponent(`Appointment request from ${form.name}`);
  const bodyLines = [
    `Name: ${form.name}`,
    `Email: ${form.email}`,
  ...
  window.location.href = `mailto:info@newham.tech?subject=${subject}&body=${body}`;
};
</script>
